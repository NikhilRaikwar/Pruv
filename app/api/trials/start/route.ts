...
export async function POST(request: Request) {
  ...
  if (trialError || !trial) {
    return NextResponse.json({ error: 'ProofLink not found' }, { status: 404 });
  }
  ...
  if (insertError || !participant) {
    return NextResponse.json({ error: 'Could not start proof' }, { status: 500 });
  }
  ...
  return NextResponse.json({
    ...
    next: `/p/${trial.slug}/scan?type=baseline`,
  });
}
